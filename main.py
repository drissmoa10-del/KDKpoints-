...
async def play_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # تم تحويل الرابط ليفتح مباشرة في المتصفح الخارجي متجاوزاً مشاكل تسجيل الدخول داخل تيليجرام
    web_app_url = f"https://lambent-axolotl-3691d8.netlify.app/?token={token}"

    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("🎮 افتح اللعبة في المتصفح الخارجي", url=web_app_url)],
        [InlineKeyboardButton("🔙 القائمة الرئيسية", callback_data='main_menu')]
    ])

    await query.edit_message_text(
        f"👑 **مملكة KDK Point الملكية**\n\n"
        f"اضغط على الزر أسفله لفتح اللعبة في المتصفح ولعبها مباشرة بدون مشاكل! 🚀",
