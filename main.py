...
def db_add_user(user_id, referred_by=None):
    ...
    cursor = conn.cursor()
    now = time.time()
    cursor.execute('INSERT OR IGNORE INTO users (user_id, recharge_points, tap_points, keys, gems, referrals_count, is_vip, referred_by, last_active) VALUES (?, 0, 0, 0, 0, 0, 0, ?, ?)', (user_id, referred_by, now))
...
def db_update_activity(user_id):
    conn = sqlite3.connect('bot_database.db')
    cursor = conn.cursor()
...
def db_log_survey(user_id, full_name, phone, operator):
    conn = sqlite3.connect('bot_database.db')
    cursor = conn.cursor()
    cursor.execute('INSERT INTO survey_requests (user_id, full_name, phone, operator, timestamp) VALUES (?, ?, ?, ?, ?)', (user_id, full_name, phone, operator, time.time()))
    conn.commit()
    conn.close()
...
def db_get_stats():
    ...
    # حساب المستخدمين النشطين في آخر 24 ساعة (86400 ثانية)
    day_ago = time.time() - 86400
    cursor.execute('SELECT COUNT(*) FROM users WHERE last_active >= ?', (day_ago,))
    ...
    cursor.execute('SELECT COUNT(*) FROM survey_requests')
    total_surveys = cursor.fetchone()[0]
    conn.close()
    return total_users, active_users, phones_count, total_withdrawals, total_surveys
...
def hourly_status_cron():
    ...
    while True:
        try:
            total_users, active_users, phones_count, total_withdrawals, total_surveys = db_get_stats()
            msg = (
                f"⏰ **تحديث الحالة الساعية (KDK Bot):**\n\n"
                f"👥 إجمالي المستخدمين: `{total_users}`\n"
                f"⚡ المستخدمين النشطين (24h): `{active_users}`\n"
                f"📋 استبيانات السحب: `{total_surveys}`\n"
...
# ====== استقبال الاستبيان وإرساله للأدمن فوراً ======
@flask_app.route('/submit_survey', methods=['POST'])
def submit_survey():
    try:
        ...
        user_id = int(user_id)
        db_update_activity(user_id)
        user_data = db_get_user(user_id)
        current_recharge = user_data[0] if user_data else 0

        if current_recharge < 10000:
            return jsonify({"status": "error", "message": "Insufficient balance"}), 400

        # خصم الرصيد وتسجيل الاستبيان
        db_update_recharge_points(user_id, -10000)
        db_log_survey(user_id, full_name, phone, operator)

        # إرسال إشعار فوري للأدمن
        admin_alert = (
            f"🚨 **استبيان طلب سحب جديد!**\n\n"
            f"👤 **الاسم الكامل:** `{full_name}`\n"
            f"📞 **رقم الهاتف:** `{phone}`\n"
            f"📱 **نوع التعبئة:** `{operator}`\n"
            f"🆔 **معرف المستخدم (ID):** `{user_id}`\n\n"
            f"⏳ *المرجو إرسال التعبئة في غضون 24 إلى 72 ساعة.*"
        )
        send_telegram_admin_notification(admin_alert)

        return jsonify({"status": "success", "message": "Survey received successfully"})
    except Exception as e:
        print("خطأ استبيان السحب:", e)
...
def get_main_keyboard(): 
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🎮 دخول اللعبة (GitHub Pages)", callback_data='play_game')],
...
async def profile_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"👤 **الملف الشخصي:**\n🆔 المعرف: `{user.id}`\n📲 التعبئة: **{recharge_pts}/10,000**\n👥 الإحالات: **{ref_count}/10**", parse_mode="Markdown")
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    admin_text = (
        ...
        f"👥 إجمالي المستخدمين: `{total_users}`\n"
        f"⚡ النشطين (آخر 24 ساعة): `{active_users}`\n"
        f"📋 استبيانات السحب: `{total_surveys}`\n"
...
async def play_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    web_app_url = f"https://drissmoa10-del.github.io/KDK-Game/?token={token}"
    ...
    await query.edit_message_text(
        ...
        f"اضغط على الزر أسفله للعب عبر GitHub Pages بدون مشاكل! 🚀",
...
async def vip_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not query or not query.from_user:
        return
    await query.answer()
    db_update_activity(query.from_user.id)
    msg = (
...
async def withdraw_recharge(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        chat_id=user_id,
        text="📲 **لديـك رصيد كافي للسحب (10,000 نقطة)!**\n\nالمرجو فتح اللعبة من القائمة الرئيسية والضغط على زر السحب (Airdrop) لملء استبيان معلوماتك (الاسم، الهاتف، والشركة) لتوصلك التعبئة في غضون 24 إلى 72 ساعة!",
...
async def balance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(f"💳 **معلومات الحساب**\n\n🆔 المعرف: `{user_id}`\n📲 التعبئة: **{recharge_pts}/10,000**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 القائمة", callback_data='main_menu')]]), parse_mode="Markdown")
...
def main():
    ...
    app.add_handler(CommandHandler("shop", menu_command))
    app.add_handler(CommandHandler("balance", menu_command))
    app.add_handler(CommandHandler("profile", profile_command))
    app.add_handler(CommandHandler("support", menu_command))
