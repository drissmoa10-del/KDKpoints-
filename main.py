...
# دوال تقرير الحالة الساعي عبر Thread مستقل
...
# مسار لإبقاء السيرفر نشطاً واستقبال طلبات الـ Ping الخارجية
...
async def earn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    safe_ad_link = f"https://t.me/share/url?url={selected_ad}"

    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("📢 مشاهدة الإعلان المباشر 🔥", url=safe_ad_link)],
        [InlineKeyboardButton("🔙 القائمة", callback_data='main_menu')]
    ])

    await query.edit_message_text(
        f"✅ **شاهد الإعلان للحصول على نقاط التعبئة:**\n\n{selected_ad}",
        reply_markup=keyboard,
        parse_mode="Markdown"
    )
...
def main():
    ...
    threading.Thread(target=hourly_status_cron, daemon=True).start()  # تشغيل التحديث الساعي في الخلفية
